...
app = Flask(__name__)
UPLOAD_PATH = "/Users/mneary1/Desktop/IPT/src/code/uploads"
...
def index():
    ...
    try:
        ...
        extraneous_lines = new_new_thesis.run_thesis(target, inputs, goals)
        with open(target) as f:
            code = f.readlines()
            #codes = code.split("\n")
            for i in range(1, len(code) + 1):
                code[i - 1] = "[" + str(i) + "] " + code[i - 1].strip("\n").replace("\t", "&emsp;" * 4).replace(" ", "&nbsp;") + "<br>"
                if i in extraneous_lines:
                    code[i - 1] = "<mark>{}</mark>".format(code[i - 1])

            results = "\n".join(code)

            return render_template("result.html", results=results, prev=code, name=os.path.split(target)[1], execute=INCLUDE_EXECUTE, semantic=INCLUDE_SEMANTIC, structural=INCLUDE_STRUCTURE)
